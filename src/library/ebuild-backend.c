...
typedef struct epkg** (*process_entry_func)(struct dirent *, int *, struct epkg **, PackageData **);
 ...
 */
struct epkg** process_directory(DIR *dir, process_entry_func process_entry,
...
struct epkg** process_pkgdir(int *packages, struct epkg **vdbpackages, PackageData **data) {
	...
	if (asprintf(&catpkgver, "%s/%s", (*data)->category, (*data)->package) < 0) {
		perror("asprintf");
		msg(LOG_ERR, "Unable to create PVF string.");
		return vdbpackages;
	}
...
struct epkg** process_vdb_package(struct dirent *pkgdp, int *packages, struct epkg **vdbpackages, PackageData **data) {
	...
	if((*data)->package == NULL) {
		perror("strdup");
		msg(LOG_ERR, "Failed to record package name!");
		abort();
	}
	...
	for (int j = 0; j < packages; j++) {
		...
		for (int k = 0; k < package->files; k++) {
			...
			if (exclude_path(file->path)) {
				free(package->content[k].path);
				free(package->content[k].md5);
				continue;
			}
